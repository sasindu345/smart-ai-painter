...
class GrokVisionProvider(VisionProvider):
    """xAI Grok Vision implementation of VisionProvider."""
    ...
    async def analyze(self, sketch_base64: str) -> SceneAnalysis:
        ...
        try:
            ...
            if not api_key:
                logger.warning("GROK_API_KEY is not set. Grok vision provider will fail.")
                raise ValueError("GROK_API_KEY config is empty.")
            ...
            payload = {
                "model": settings.grok_model or "grok-2-vision-1212",
                "messages": [
                    {
                        "role": "system",
            ...
            if response.status_code != 200:
                logger.error(f"Grok API returned error status {response.status_code}: {response.text}")
                raise ValueError(f"Grok API error: {response.text[:200]}")
            ...
            logger.error("Grok VLM analysis failed: %s", str(e), exc_info=True)
            ...
            return SceneAnalysis(
                ...
                view="unknown",
                confidence=0.0,
                raw_description=f"Grok VLM error: {str(e)[:300]}",
            )
